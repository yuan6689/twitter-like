  ...
  <div>
    <div class="flex items-center flex-shrink-0 p-4 pb-0">
      ...
      <div class="w-full p-2">
        <textarea
          v-model="text"
          class="w-full h-14 text-lg text-gray-900 placeholder:text-gray-400 bg-transparent border-0 dark:text-white focus:ring-0"
    ...
    <!-- file Selector -->
    <div class="p-4 pl-16">
      <template v-for="i in inputImageUrl" v-if="inputImageUrl.length > 0">
        <img :src="i" class="rounded-2xl border" :class="twitterBorderColor" />
      </template>
      <input
        type="file"
    ...
    <div class="flex p-2 pl-14">
      ...
      <div class="flex w-full text-white">
        <div
          class="p-2 text-blue-400 rounded-full hover:bg-blue-50 dark:hover:bg-dim-800 hover:cursor-pointer"
        >
          <!-- Image frame Icon -->
          <svg
            viewBox="0 0 24 24"
...
const text = ref("");
const imageInput = ref(""); // 绑定的 type=file 的 input
const selectedFile = ref([]);
const inputImageUrl = reactive([]);
...
const handleFileInputChange = async (event) => {
  ...
  for (let i = 0; i < files.length; i++) {
    inputImageUrl.push(await readFileAsync(files[i]));
  }
};
</script>
